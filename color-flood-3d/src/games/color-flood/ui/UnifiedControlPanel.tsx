...
export const UnifiedControlPanel: React.FC<UnifiedControlPanelProps> = ({ className = '' }) => {
  ...
  const difficultyOptions = useMemo(() => {
    return LEVEL_TIERS.filter(tier => tier.id !== 'tutorial').map(tier => ({
      ...
      name: tier.name,
      color: tier.color,
      count: SAMPLE_LEVELS.filter(level => level.tier === tier.id).length
    }));
  }, []);
      ...
      <div className="control-section difficulty-section">
        ...
        <div className="difficulty-radio-group">
          {difficultyOptions.map(option => (
            <label key={option.id} className="difficulty-option">
              <input
                type="radio"
                name="difficulty"
                value={option.id}
              ...
              <span className="difficulty-count">({option.count})</span>
            </label>
          ))}
        </div>
